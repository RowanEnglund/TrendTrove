# TrendTrove Specification Documents

This directory contains the full specification documents for the TrendTrove project, covering both the Android app and the website.

## Directory Structure

-   `android/`: Contains specifications unique to the Android application.
-   `website/`: Contains specifications unique to the website, including the administration layer.
-   `shared/`: Contains specifications that are common to both the Android app and the website.

## Documents

### Shared

-   `authentication.md`: User account creation, login, and profile management.
-   `payments.md`: Payment gateway integration, pricing, and order placement.
-   `products.md`: Product discovery, details, and user preferences.
-   `dropshipping.md`: Dropshipping model and order fulfillment.

### Android

-   `ui_ux.md`: Android app UI/UX design and push notifications.

### Website

-   `admin_panel.md`: Website administration layer for analytics and management.
